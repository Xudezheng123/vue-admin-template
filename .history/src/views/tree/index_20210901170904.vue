...
export default {
  ...
  methods: {
    ...
    handleDrop(draggingNode, dropNode, dropType, ev) {
      ...
      // let dragId = draggingNode.data.id;
      if(dropType == "before"||dropType == "after"){
        let pId = dropNode.data.parentId;
        let listId = dropNode.parent.childNodes.reduce((prev,cur,index,array)=>{
          console.log("cur",cur.data.id);
          console.log("prev",prev);
          prev.push(cur.data.id);
        },[]).join(',')
      }else{
        let pId = dropNode.data.Id;
        let listId = dropNode.data.children.reduce((prev,cur,index,array)=>{
          console.log("cur:",cur.data.id);
          console.log("prev",prev);
          prev.push(cur.data.id);
        },[]).join(',')
      }
      console.log(dragId,pId,listId);
    },
    onSubmit() {
