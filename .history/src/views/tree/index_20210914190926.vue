    ...
    <el-row :gutter="20">
      ...
      <el-col :xs="15" :sm="18" :md="19" :lg="20" :xl="20">
        ...
        <el-form ref="form" :model="form" label-width="80px">
          <el-form-item label="菜单ID" v-show="form.id && form.id!=0">
          ...
          <el-form-item label="菜单父ID" prop="parentId">
            <treeselect
              ...
              :show-count="true"
            >
              <div slot="value-label" slot-scope="{ node }">{{ node.raw.id }} {{ node.raw.name }}</div>
          ...
          <el-form-item label="权限ID">
            <el-select v-model="form.permCode" multiple placeholder="请选择权限ID" style="width:500px">
              <el-option
          ...
          <el-form-item label="菜单图标" prop="icon">
            ...
            >
              ...
              <el-input slot="reference" v-model="form.icon" style="width: 700px;" placeholder="点击选择图标" readonly>
                <span v-if="form.icon" slot="prefix" :class="iconClassName" class="el-input__icon" style="height: 32px;width: 16px;" />
...
<script>
/* eslint-disable */
import {getAllMenu ,deleteMenus ,getMenuDetail ,addMenu, moveMenu,getAllPermissions} from '@/api/table'
  ...
  methods: {
    ...
    handleDrop(draggingNode, dropNode, dropType, ev) {
      ...
      if(dropType == "before"||dropType == "after"){
        ...
        listId = dropNode.parent.childNodes
        .reduce((pre,cur,index,array)=>{
          pre.push(cur.data.id)
          return pre;
        },[]).join(',');
      }else{
        pId = dropNode.data.id;
        listId = dropNode.childNodes
        .reduce((prev,cur,index,array)=>{
          prev.push(cur.data.id);
          return prev;
        },[]).join(',');
      }
      moveMenu({
    ...
    onSubmit() {
      this.form.permCode = this.form.permCode.join(',')
    ...
    deleteSelected(){
      this.$confirm('确定删除？','提示',{
        ...
        deleteMenus(this.form.id).then(res=>{
          this.$message({message:'操作成功',type:'success'});
          this.getMenuDatas();
          this.newAdd();
        })
