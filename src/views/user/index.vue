<template>
    <div class="app-container">
        ...
        <el-dialog
            ...
            <el-row style="margin:10px 0 0 310px">
    ...
    methods:{
        ...
        getAllRole(){
            getRoleList(this.queryRole).then(res=>{
                this.role = res.data.reduce((pre,cur,index,arr)=>{
                    let obj = {
                        id:cur.id,
                        name:cur.name
                    }
        ...
        handleTransfer(){
            this.transfer.roleIds = this.transfer.roleIds.join(',');
            bindRole(this.transfer).then((res)=>{
        ...
        handleRole(index,row){
            ...
            this.transfer.id = row.managerId;
            console.log("row",row);
            getUserRole(row.managerId).then((res)=>{
                console.log("res",res)
                this.transfer.roleIds = res.data.reduce((pre,cur,index,arr)=>{
                    pre.push(cur.id);
                    return pre;
                },[]);
                console.log("roleIds",this.transfer.roleIds)
            }).catch(e=>{})
        },
